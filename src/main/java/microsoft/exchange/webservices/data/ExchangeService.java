...
IAutodiscoverRedirectionUrl {
	 ...
	 * @param deleteMode
	 *            The delete mode
	 * @param deleteSubFolders if set to <c>true</c> empty 
	...
	public ServiceResponseCollection<ServiceResponse> createItems(
        ...
        // E14:298274 Make sure that all items do *not* have unprocessed attachments.
        if (!EwsUtilities.trueForAll(items,new IPredicate<Item>() {				
			@Override
			public boolean predicate(Item obj) throws ServiceLocalException {
	 ...
	 * @return A ServiceResponseCollection providing
	 *  copy results for each of the specified item Ids.	
	 * @throws Exception 
	 */	
	public ServiceResponseCollection<MoveCopyItemResponse> copyItems(
			Iterable<ItemId> itemIds,
	 ...
	 *             the exception
	 */
	protected <TItem extends Item> ServiceResponseCollection
	<FindItemResponse<TItem>> 
	findItems(Iterable<FolderId> parentFolderIds, SearchFilter searchFilter,
	 ...
	 */
	protected <TItem extends Item> ServiceResponseCollection
	<FindItemResponse<TItem>> findItems(
	...
	public FindItemsResults<Appointment> findAppointments(
		...
		ServiceResponseCollection<FindItemResponse<Appointment>> response = this.findItems(
				folderIdArray, 
				null, /* searchFilter */
				null /* queryString */, 
				calendarView, 
	 ...
	 *             the exception
	 */
	private ServiceResponseCollection<GetAttachmentResponse> 
	internalGetAttachments(Iterable<Attachment> attachments,
			BodyType bodyType,
	...
	throws Exception {
		...
		return internalSubscribeToPullNotifications(null, timeout, watermark,
				eventTypes);
	}
	...
	public PullSubscription subscribeToPullNotifications(
		...
		return internalSubscribeToPullNotifications(folderIds, timeout,
				watermark, eventTypes);
	}
	 ...
	 *             the exception
	 */
	private PullSubscription internalSubscribeToPullNotifications(
			Iterable<FolderId> folderIds, int timeout, String watermark,
			EventType[] eventTypes) throws Exception {
		...
		SubscribeToPullNotificationsRequest request = 
			new SubscribeToPullNotificationsRequest(
					this);
		...
		return request.execute().getResponseAtIndex(0).getSubscription();
	...
	throws Exception {
		...
		return request.execute().getResponseAtIndex(0).getResults();
	}
	...
	public PushSubscription subscribeToPushNotifications(
		...
		EwsUtilities.validateParamCollection(folderIds.iterator(), "folderIds");
		return internalSubscribeToPushNotifications(folderIds, url, frequency,
				watermark, eventTypes);
	}
	...
	throws Exception {
		...
		return internalSubscribeToPushNotifications(null, url, frequency,
				watermark, eventTypes);
	}

	/**
	 * * Subscribes to PushNotifications. If folderIds is null, we subscribe to
	 * all notifications on a mailbox.
	 * 
	 * @param folderIds
	 *            the folder ids
	 * @param url
	 ...
	 *             the exception
	 */
	private PushSubscription internalSubscribeToPushNotifications(
			Iterable<FolderId> folderIds, URI url, int frequency,
			String watermark, EventType[] eventTypes) throws Exception {
		...
		EwsUtilities.validateParamCollection(eventTypes, "eventTypes");
		SubscribeToPushNotificationsRequest request = 
			new SubscribeToPushNotificationsRequest(
					this);
	...
	/**
	 * Subscribes to streaming notifications on all 
	 * folders in the authenticated user's mailbox. 
	 * Calling this method results in a call to EWS.	 
			...
			EventType... eventTypes) throws Exception
	{
		...
		return this.internalSubscribeToStreamingNotifications(
				null /* all folders*/, 
				eventTypes);
	}
	...
	/**
	 * Subscribes to StreamingNotifications.  If folderIds is null, 
	 * we subscribe to all notifications on a mailbox.	  
	 * @param folderIds The Ids of the folder to subscribe to.	 
	 * @param eventTypes The event types to subscribe to.
	 * @return A StreamingSubscription representing the new subscription.	 
	 * @throws Exception 
	 */   
	private StreamingSubscription internalSubscribeToStreamingNotifications(
			Iterable<FolderId> folderIds,
			EventType[] eventTypes) throws Exception
			{
		EwsUtilities.validateParamCollection(eventTypes, "eventTypes");
	...
	public ChangeCollection<ItemChange> syncFolderItems(FolderId syncFolderId,
		...
		return request.execute().getResponseAtIndex(0).getChanges();
	}
	...
	public ChangeCollection<FolderChange> syncFolderHierarchy(
			...
			throws Exception {
		EwsUtilities.validateParamAllowNull(syncFolderId, 
		"syncFolderId"); // Null
		// syncFolderId
		// is
		// allowed
	 ...
	 * @param filter The search filter. Only search 
	 * filter class supported SearchFilter.IsEqualTo
	 * @param folderId The Id of the folder in which to search for conversations.	
	 * @throws Exception 
	 */	
	private Collection<Conversation> findConversation(ConversationIndexedItemView view, 
			SearchFilter.IsEqualTo filter, FolderId folderId) throws Exception
	{
		EwsUtilities.validateParam(view, "view");
	...
	public Collection<Conversation> findConversation(
			ConversationIndexedItemView view,
	 ...
	 * @param processRightAway True to process at once . This is blocking 
	 * and false to let the Assitant process it in the back ground
	 * @param categories Catgories that need to be stamped can be null or empty
	 * @param enableAlwaysDelete True moves every current and future messages 
	 * in the conversation to deleted items folder. 
	 * False stops the alwasy delete action. This is 
	 * applicable only if the action is AlwaysDelete
	 * @param destinationFolderId Applicable if the action is AlwaysMove. 
	 * This moves every current message and future
	 * message in the conversation to the specified folder. 
	 * Can be null if tis is then it stops
	...
	private ServiceResponseCollection<ServiceResponse> applyConversationAction(
			...
			boolean processRightAway,
			StringList categories,
			boolean enableAlwaysDelete,
			FolderId destinationFolderId,
			ServiceErrorHandling errorHandlingMode) throws Exception {
		...
		ApplyConversationActionRequest request = 
			new ApplyConversationActionRequest(this, errorHandlingMode);
		...
		for(ConversationId conversationId : conversationIds)
		{
			...
			action.setDestinationFolderId(destinationFolderId != null ? 
					new FolderIdWrapper(destinationFolderId) : null);
	...
	/**
	 * Applies one time conversation action on items 
	 * in specified folder inside the conversation.
	...
	private ServiceResponseCollection<ServiceResponse> applyConversationOneTimeAction(
		...
		for(HashMap<ConversationId, Date> idTimePair : idTimePairs)
		{
			...
			action.setContextFolderId(contextFolderId != null ? 
					new FolderIdWrapper(contextFolderId) : null);
			action.setDestinationFolderId(destinationFolderId != null ? 
					new FolderIdWrapper(destinationFolderId) : null);
			action.setConversationLastSyncTime(
					idTimePair.values().iterator().next());
	 ...
	 * @param categories The categories that should be stamped on 
	 ...
	 * within that conversation is no longer categorized.
	 * Calling this method results in a call to EWS.
	 * @param conversationId The id of the conversation.	
	 * @param processSynchronously Indicates whether the method should
	 *  return only once enabling this rule and stamping existing items
	...
	/**
	 * Sets up a conversation so that any item received 
	 * within that conversation is no longer moved to Deleted Items folder.
	 * Calling this method results in a call to EWS.
	 * @param conversationId The id of the conversation.	
	 * @param processSynchronously Indicates whether the method should 
	 * return only once enabling this rule and stamping existing items
	...
	/**
	 * Sets up a conversation so that any item 
	 * received within that conversation is always moved to a specific folder.
	 * Calling this method results in a call to EWS.
	...
	/**
	 * Sets up a conversation so that any item received 
	 * within that conversation is no longer moved to a specific folder.
	 * Calling this method results in a call to EWS.
	 * @param conversationIds The conversation ids.
	 * @param processSynchronously Indicates whether the method should 
	...
	/**
	 * Moves the items in the specified conversation
	 ...
	 * that dateTime will not be moved).
	...
	/**
	 * Copies the items in the specified conversation 
	 * to the specified destination folder.
	 * Calling this method results in a call to EWS.
	 * @param idLastSyncTimePairs The pairs of Id of conversation whose items
	...
	public ServiceResponseCollection<ServiceResponse> deleteItemsInConversations(
			Iterable<HashMap<ConversationId, Date>> idLastSyncTimePairs,
			FolderId contextFolderId,
			DeleteMode deleteMode) throws Exception	{
		return this.applyConversationOneTimeAction(
	 ...
	 * Calling this mehtod would	result in call to EWS.
	 * @param idLastSyncTimePairs The pairs of Id of conversation whose 
	 ...
	 * dateTime will not be copied).
	 * @param contextFolderId The Id of the folder that contains the conversation.
	 * @param isRead if set to <c>true</c>, conversation items are 
	 * marked as read; otherwise they are marked as unread.
	 * @throws Exception 
	 * @throws Exception 
	 */   
	...
	throws Exception {
		...
		CreateUserConfigurationRequest request = 
			new CreateUserConfigurationRequest(
					this);
	...
	throws Exception {
		...
		DeleteUserConfigurationRequest request = 
			new DeleteUserConfigurationRequest(
					this);
	 ...
	 * @return A RuleCollection object containing 
	 * the authenticated users inbox rules.	
	 * @throws Exception 
	 */    
	 ...
	 * @return A RuleCollection object containing
	 *  the inbox rules of the specified user.
	 * @throws Exception 
	 */
	...
	/**
	 * Updates the authenticated user's inbox rules
	 *  by applying the specified operations.
		...
		if (this.getRequestedServerVersion().ordinal() > 
			...
			try {
				...
				.traceMessage(
						...
						String
						.format(
								"Autodiscover service call " +
								"failed with error '%s'. " +
								"Will try legacy service",
								ex.getMessage()));
	...
	throws Exception {
		...
		AutodiscoverService autodiscoverService = 
			new AutodiscoverService(this,requestedServerVersion);
		autodiscoverService
		...
		switch (response.getErrorCode())
        {
            case NoError:
                return this.getEwsUrlFromResponse( response, 
                		autodiscoverService.isExternal().TRUE);
            ...
            case InvalidRequest:
                throw new ServiceRemoteException(
                    String.format(Strings.InvalidAutodiscoverRequest, 
                    		response.getErrorMessage()));

            default:
                this.traceMessage(
                    TraceFlags.AutodiscoverConfiguration,
                    String.format("No EWS Url returned for user %s, " +
                    		"error code is %s", emailAddress, 
	...
	throws URISyntaxException, AutodiscoverLocalException
	{
		...
		if ((isExternal && 
				response.tryGetSettingValue(String.class,
		...
		throw new AutodiscoverLocalException(Strings.
				AutodiscoverDidNotReturnEwsUrl);
	}
	...
	protected Document executeDiagnosticMethod(String verb, 
	  Node parameter) throws Exception
	{
		ExecuteDiagnosticMethodRequest request = 
		new ExecuteDiagnosticMethodRequest(this);
	 ...
	 * targeting the specified version of EWS and
	 *  scoped to the to the system's current time zone. 	
	 */		
	 ...
	 * Initializes a new instance of the <see cref="ExchangeService"/> class,
	 * targeting the specified version of EWS and 
	 * scoped to the to the specified time zone.	 
	...
	throws ServiceLocalException, URISyntaxException {
		return this.prepareHttpWebRequestForUrl(this.getUrl(), this
	 ...
	 * AutodiscoverUrl method should perform SCP 
	 * (Service Connection Point) record lookup when
	 *  determining the Autodiscover service URL.	
	 */
	 ...
	 * Exchange2007 compatibility mode is enabled.
	 * <remarks>
	 * In order to support E12 servers, the 
	 * Exchange2007CompatibilityMode property can be used 
	 * to indicate that we should use "Exchange2007" 
	 * as the server version string rather than 
	 * Exchange2007_SP1.
	 * </remarks>
	 */
	...
	public Collection<TimeZoneDefinition> getServerTimeZones(
		...
		Date today = new Date();
		Collection<TimeZoneDefinition> timeZoneList = 
